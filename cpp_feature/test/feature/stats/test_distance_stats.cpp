...
#include <feature/stats/distance_stats.hpp>

using namespace feature;
using namespace feature::details;

TEST_CASE("Test distance_stats::distance_stats", "[distance_stats]") {

    SECTION("Test with multiple players") {
        ...
        SECTION("Check if the results are written to correct indices") {
            distance_stats(players.begin(), players.end(), "home", features);
            REQUIRE(features[name_to_index("homeInnerDistance")] != -10);

            distance_stats(players.begin(), players.end(), "away", features);
            REQUIRE(features[name_to_index("awayInnerDistance")] != -10);
        }

        SECTION("Check for correct results") {
            distance_stats(players.begin(), players.end(), "home", features);
            REQUIRE(features[name_to_index("homeInnerDistance")] ==
                    Approx(33537.0177));
        }
    }

    SECTION("Test with a single player") {
        std::vector<Player> players{Player(123, 456)};
        std::vector<double> features(num_features(), -10);

        distance_stats(players.begin(), players.end(), "home", features);
        REQUIRE(features[name_to_index("homeInnerDistance")] == 0);
    }

    SECTION("Test with no players") {
        std::vector<Player> players;
        std::vector<double> features(num_features(), -10);
